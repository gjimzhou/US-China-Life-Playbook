...
await p.evaluate(()=>Object.defineProperty(navigator,'clipboard',{value:{writeText:async()=>{throw Error()}},configurable:true}));await p.getByRole('button',{name:'复制小节链接'}).first().click();assert(await p.getByText('无法自动复制，请手动复制已选中的地址。').isVisible());
...
await p.locator('#backup-file').setInputFiles({name:'bad.json',mimeType:'application/json',buffer:Buffer.from('{broken')});assert(await p.getByText('文件格式无效、版本不支持或文件过大。没有更改任何本地数据。').isVisible());
