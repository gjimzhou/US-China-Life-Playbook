...
window.Reader = (() => {
 ...
 function restoreProgress(p){if(!storageOK){message('进度数据不可读，原数据未覆盖。');return false}try{if(localStorage.getItem(key)!==progressRaw){readProgress();resume();message('进度已发生变化，请检查后重试。');return false}const raw=p?JSON.stringify(p):null;if(raw)localStorage.setItem(key,raw);else localStorage.removeItem(key);progressRaw=raw;progress=p;resume();message('阅读位置已恢复。');return true}catch{message('阅读位置恢复失败，原数据未更改。');return false}}
 return {restoreProgress,stableRoute,event,beforeRender,mount,updates,privacy,trackView,async init(ds){documents=ds;readProgress();resume();try{const r=await fetch('services.json',{signal:AbortSignal.timeout(3000)});if(r.ok){const next=await r.json();if(next.analytics&&next.comments&&next.newsletter)config=next}}catch{}initAnalytics()}};
})();
