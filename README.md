# US–China Life Playbook

> 面向高收入、长期在美国生活、同时维持中国家庭与社会连接的第一代移民家庭的一套可执行生活操作系统。

这不是“省钱技巧合集”，也不是“中国人如何适应美国”的入门手册。它假设读者已经能够独立生活、受教育程度较高、家庭收入较高，也愿意用钱购买专业服务；真正缺少的是美国本地家庭通过父母、朋友、社区和几十年生活经验自然获得的 **tacit knowledge / social infrastructure**，以及一套能同时维持中美两地生活的系统。

核心目标：

1. 降低死亡、重伤、诉讼、身份盗窃、税务、移民、失能等 **tail risk**。
2. 建立 “event → institution → professional → procedure” 的默认映射。
3. 用专业服务和制度替代临时 Google、记忆和运气。
4. 让美国成为完整的 principal home system，同时保持中国作为可随时重新接入的 secondary home system。
5. 把复杂家庭生活变成一套可以维护、审计和年度 review 的 operating system。

---

## 适用画像

本指南主要为以下家庭设计：

- 30–50 岁，第一代移民或长期海外华人；
- 美国为主要居住地，同时每年在中国停留并维护父母、身份、银行、医疗和社会关系；
- 高 W-2 / professional income，有显著储蓄和资产；
- 有能力购买律师、CPA、insurance broker、医生、contractor 等专业服务；
- 有汽车、旅行、宠物、婚姻、未来孩子/父母养老等复杂 household operations；
- 对美国很多“默认大家都知道”的社会流程并没有代际传承。

它不是根据收入决定“谁更值得安全”，而是承认：不同家庭的 opportunity cost、资产暴露、专业服务使用方式和跨境复杂度不同。

---

## 本书的基本哲学

### 1. 用钱购买可靠性，而不是购买炫耀

高收入以后，$100–$1,000 的优化通常不如一次六位数 liability、错误税务处理、身份盗窃、漏掉保险、重大医疗延误重要。

### 2. 优先处理不可逆 downside

排序优先考虑：

- impact
- probability
- irreversibility
- evidence
- implementation cost
- ongoing friction

### 3. 不把“检查越多”误认为“健康管理越好”

医疗以 evidence-based preventive care 为基线。无指征筛查、全身扫描、肿瘤标志物套餐等需要同时考虑 false positive、incidental finding、overdiagnosis 和 downstream harm。

### 4. 不要求家庭成员记住所有东西

最可靠的家庭不是“最聪明”，而是：

- 有 checklist；
- 有固定入口；
- 有备用联系人；
- 有文档；
- 有明确授权；
- 有年度 review。

### 5. 中美双栖不是两个半套系统

目标是：

**US primary system + China maintained secondary system + cross-border interface**

而不是每次回国重新学习中国、每次返美重新 Google 美国。

---

## 每条建议的标签

计划逐步为建议加上以下标签：

| 字段 | 含义 |
|---|---|
| Evidence | A / B / C：证据和权威来源质量 |
| Priority | P0 / P1 / P2 / P3 |
| Impact | Low / Medium / High / Catastrophic |
| Cost | Free / Low / Medium / High |
| Friction | One-time / Annual / Ongoing |
| Geography | US / China / Cross-border |
| Owner | Self / Spouse / Household / Professional |

### Priority

- **P0**：低成本、高后果，建议尽快完成。
- **P1**：明显提高家庭稳健性，建议本年度完成。
- **P2**：在特定生活阶段执行。
- **P3**：有价值但不急。

详见 [METHODOLOGY.md](METHODOLOGY.md)。

---

## 目录

### A. 生存、健康与医疗

1. [不要意外死亡](book/01-不要意外死亡.md)
2. 预防医学：该做什么，不该做什么
3. 紧急情况与急救
4. 美国医疗系统怎么用
5. 中国医疗系统怎么用
6. 医疗记录、药物和跨境 continuity of care

### B. 风险、保险与法律

7. 保险与灾难风险
8. 汽车事故、停车、维修与责任
9. 住房、租房、物业与 contractor
10. 婚姻、prenup 与夫妻财务
11. Estate planning、失能与死亡
12. 律师、CPA、保险经纪怎么找

### C. 钱、税与数字资产

13. 高收入家庭财务 operating system
14. 美国税务与 IRS
15. 中美跨境税与 residency
16. 银行、现金与账户结构
17. 网络安全、身份盗窃与 digital estate

### D. 美国社会 operating system

18. [美国政府到底找谁](book/11-美国政府到底找谁.md)
19. [美国人默认知道但第一代移民没人教的事](book/31-美国人默认知道的100件小事.md)
20. 美国社交礼仪与 social conventions
21. 如何建立本地 professional / neighbor / community network
22. 消费纠纷、诈骗、small claims 与 regulator

### E. 中国 secondary home system

23. 中国身份、手机号、银行、微信/支付宝
24. 中国政府、医院和办事入口
25. 如何维持中国社会关系
26. 父母养老、远程照护与 emergency response

### F. 中美接口

27. [中美双栖基础设施](book/18-中美双栖生活基础设施.md)
28. 文件、公证、Apostille 与授权
29. 旅行、长期停留与全球 mobility
30. 双边资产、保险和医疗 continuity

### G. Household operations

31. 宠物
32. 婚礼、孩子与家庭生命周期
33. 时间、精力与 household management
34. 重大打击后的 temporary operating mode
35. Emergency Runbook
36. 年度家庭 review
37. 反面清单：有钱也不要乱做的事

---

## 快速开始

如果只做五件事：

1. 建立夫妻共享的 household master folder。
2. 填写 [家庭 Emergency Sheet](checklists/emergency-sheet.md)。
3. 做一次 [年度家庭 Review](checklists/annual-review.md)。
4. 给美国和中国分别建立医疗、法律、财务、住房和紧急联系人入口。
5. 把高后果风险从“以后再说”改成有 owner、有日期、有复查频率的 checklist。

---

## 资料来源

本项目优先使用：

1. 美国/中国官方机构；
2. 专业协会 guideline；
3. systematic review / meta-analysis；
4. 高质量 observational evidence；
5. 对社会习俗、实际流程等缺乏 RCT 的领域使用明确标注的 practical guidance。

具体政策见 [references/source-policy.md](references/source-policy.md)。

医疗、税务、法律、移民和保险规则会变化；正文中的具体阈值、期限和政策应注明来源和最后核验日期。

---

## 隐私原则

本 repo 默认保存 **通用规则和家庭模板**，不应直接提交：

- SSN / 身份证号码；
- passport number；
- 银行账号；
- 密码 / recovery code；
- 完整 medical record；
- 私人律师、医生、父母等敏感联系人详情；
- 真实住址、工作单位、收入/资产数字、行程、家庭成员身份、健康史、移民个案等可识别个人信息。

真实敏感信息应存放在合适的 password manager、encrypted storage 或其他私密系统；repo 只存字段结构和操作说明。

任何示例人物、金额、地点和情景都应使用虚构或泛化数据，不以 repo owner 的真实情况为示例。

---

## 状态

**v0.1 — 建设中**

目标体量：约 **30–40 个章节、500–700 条 actionable rules**。

与一次性文章不同，本项目设计为长期维护：事实会更新，家庭阶段会变化，优先级也会变化。

最后更新：2026-09-21
